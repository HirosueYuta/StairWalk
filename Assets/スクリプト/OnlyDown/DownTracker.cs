...
DownTracker : MonoBehaviour
{
    ...
    private float previousHeightLeftTracker;
    private float previousHeightRightTracker;
    public bool isLeftFootDown = false;// 左足が上方向に移動しているかどうかを示すフラグ
    public bool isRightFootDown = false;// 右足が上方向に移動しているかどうかを示すフラグ

     // しきい値（ノイズ除去用）
    private float upwardThreshold = 0.003f; // この値以上の高さ変化があれば移動とみなす
    ...
    void Update()
    {
        ...
        if (isInitialHeightSet)
        {
            ...
            previousHeightLeftTracker = leftTracker.position.z ;
            previousHeightRightTracker = rightTracker.position.z;
        }
    }
    ...
    void DowndateRelativeHeights(){
        ...
        Debug.Log("initialHeightRightTracker : (" +initialHeightRightTracker+ ")");

    }

    void IsFootTriger(){    
        // // 足が初期値 + 10cm 以内にいるかどうか
        // canTriggerLeft = RelativeHeightLeftTracker <= initialHeightLeftTracker + 0.2f;
        // canTriggerRight = RelativeHeightRightTracker <= initialHeightRightTracker + 0.2f;
        // //上方向に動いているかを判定
        // isLeftFootDown = (RelativeHeightLeftTracker - previousRelativeHeightLeftTracker) > upwardThreshold;
        // isRightFootDown = (RelativeHeightRightTracker - previousRelativeHeightRightTracker) > upwardThreshold;

        isLeftFootDown = leftTracker.position.z - previousHeightLeftTracker> upwardThreshold;
        isRightFootDown = rightTracker.position.z - previousHeightRightTracker> upwardThreshold;
    }

    void ProcessStepInput(){
        //通常の入力を処理
        if (isLeftFootDown && canTriggerLeft && !isRightFootNext)
        {
            isLeftShoeTurn = true;
        ...
        else if (isRightFootDown && canTriggerRight && isRightFootNext)
        {
